    ...
    async (params) => {
      try {
        ...
        const lines: string[] = [
          ...
          "---",
          "",
          "## Stage 0 — Canon & Foundation (Visual Concept 전자동 포함)",
          `> 목표: 마스터 레퍼런스 에셋 확립. 모든 이후 Stage는 이 Canon을 기준으로 생성.`,
          `> Visual Concept: 키 비주얼 후보를 구도 프리셋으로 K개 생성 → asset_select_best가 자동 선별해 canon 등록.`,
          `> 사람 선택 없음 — 전원 기준 미달(all_below_threshold) 시 후보 1회 재생성 후 재선별.`,
          ...
          `| 2 | Key Visual 후보 ×3 🔴 | 1536×1024 | \`asset_generate_image\` (KEY_VISUAL_VARIATION_PRESETS) | ⬜ |`,
          `| 3 | Key Visual 자동 선별 🔴 | — | \`asset_select_best\` (register_as_canon=true) | ⬜ |`,
          `| 4 | 대표 캐릭터/배경 분리 추출 | auto | \`asset_generate_with_reference\` (key visual 기준) | ⬜ |`,
          `| 5 | 팔레트 역주입 | — | \`asset_extract_palette\` → CONCEPT 갱신 | ⬜ |`,
          `| 6 | Style Reference Sheet 🔴 | auto | \`asset_generate_style_reference_sheet\` | ⬜ |`,
          "",
          "---",
          "",
          "## Stage 1 — Characters",
