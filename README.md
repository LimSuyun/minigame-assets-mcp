...
# (선택) Gemini 도 동일 패턴
security add-generic-password -a "$USER" -s claude-code-gemini-api-key -w "AIza..." -U
```
...
```json
{
  "mcpServers": {
    "minigame-assets": {
      ...
      "args": [
        "-c",
        "exec env OPENAI_API_KEY=\"$(security find-generic-password -a \"$USER\" -s claude-code-openai-api-key -w 2>/dev/null)\" GEMINI_API_KEY=\"$(security find-generic-password -a \"$USER\" -s claude-code-gemini-api-key -w 2>/dev/null)\" npx -y minigame-assets-mcp@latest"
      ]
    }
  }
}
