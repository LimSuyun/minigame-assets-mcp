...
MCP 서버 + 슬래시 커맨드 + 자동 발동 워크플로 스킬을 한 번에 설치합니다.
...
- **MCP 서버** `minigame-assets` — `npx -y minigame-assets-mcp` 로 자동 실행
- **슬래시 커맨드**: `/create-minigame-assets`, `/setup-minigame-assets-concept`, `/minigame-assets-help`
- **자동 발동 스킬** `minigame-assets-workflow` — "게임 에셋 만들어줘" 류 요청에 반응해 CONCEPT.md 를 먼저 생성하고 워크플로를 따르도록 유도

플러그인 사용 시에도 환경변수(`OPENAI_API_KEY`, 선택적으로 `GEMINI_API_KEY` 등) 는 사용자 쉘/Claude Code 설정에서 설정해 둬야 합니다. 아래 [선택적 환경 변수](#선택적-환경-변수) 표 참고.

플러그인 소스: [`plugins/minigame-assets/`](./plugins/minigame-assets/)

### 수동 MCP 등록 (플러그인 없이)

슬래시 커맨드 없이 MCP 서버만 쓰고 싶거나 `.env` 식으로 키를 고정 주입하고 싶을 때:
