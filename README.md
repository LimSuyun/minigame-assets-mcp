...
```bash
git clone https://github.com/your-username/minigame-assets-mcp.git
...
# 선택 (기본값 있음)
ASSETS_OUTPUT_DIR=./generated-assets   # 에셋 저장 경로
CONCEPT_FILE=./game-concept.json        # 게임 컨셉 JSON 경로
CONCEPT_MD_FILE=./CONCEPT.md            # 게임 컨셉 마크다운 경로
LOCAL_MUSIC_SERVER_URL=http://localhost:7860  # 로컬 음악 서버 URL
```
...
| `CONCEPT_FILE` | 선택 | 게임 컨셉 JSON 저장 경로 |
| `CONCEPT_MD_FILE` | 선택 | CONCEPT.md 저장 경로 |
...
### 방법 1: 프로젝트 로컬 등록 (`.mcp.json`)

게임 프로젝트 루트에 `.mcp.json` 파일 생성 (`.mcp.json.example` 복사 후 수정):
...
```json
{
  "mcpServers": {
    "minigame-assets": {
      "command": "node",
      "args": ["/절대경로/minigame-assets-mcp/dist/index.js"],
      "env": {
        "OPENAI_API_KEY": "sk-...",
        "GEMINI_API_KEY": "AIza...",
...
### 방법 2: 전역 등록 (`~/.claude/claude_desktop_config.json`)
