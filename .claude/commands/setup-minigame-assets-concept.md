...
### 캐릭터 스프라이트
1. `asset_generate_character_base` — 정면 베이스 이미지 생성
2. `asset_generate_sprite_sheet` — 액션 스프라이트 일괄 생성
   - export_formats는 2단계 분석 결과의 권장값 사용

### 무기 아이콘
`asset_generate_weapons` — gpt-image-1, 투명 배경

### 배경 / UI 이미지
`asset_batch_generate_images` — asset-plan.json의 params 값 사용
